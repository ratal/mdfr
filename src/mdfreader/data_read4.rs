...
use arrow::array::{
    BinaryArray, FixedSizeBinaryArray, FixedSizeListArray, LargeBinaryBuilder, LargeStringBuilder,
    PrimitiveBuilder,
};
        ...
        && !cn.data.is_empty()
    {
        ...
        match cn.data.data_type() {
            DataType::Int8 => {
                ...
                cn.data = PrimitiveBuilder::from_vec(buf).boxed();
            }
            DataType::UInt8 => {
                cn.data = PrimitiveBuilder::from_vec(data_bytes.clone()).boxed();
            }
            DataType::Int16 => {
                let mut buf = vec![0; cycle_count];
                if cn.endian {
            ...
            DataType::Int32 => {
                if n_bytes == 4 {
                    ...
                    let data = cn.data.as_any_mut().downcast_mut::<PrimitiveBuilder<i32>>()
                        ...
                        .values_slice_mut().context("One channel array function could not get mutable values from primitive array i32")?;
                    if cn.endian {
            ...
            DataType::UInt32 => {
                if n_bytes == 4 {
                    ...
                    let data = cn.data.as_any_mut().downcast_mut::<PrimitiveBuilder<u32>>()
                        .context("One channel array function could not downcast to primitive array u32")?
                        .values_slice_mut().context("One channel array function could not get mutable values from primitive array u32")?;
                    if cn.endian {
                        for (i, mut value) in data_bytes.chunks(n_bytes).enumerate() {
                            data[i] = value
                                .read_u24::<BigEndian>()
            ...
            DataType::Float32 => {
                if n_bytes == 4 {
                    ...
                    let data = cn.data.as_any_mut().downcast_mut::<PrimitiveBuilder<f32>>()
                        .context("One channel array function could not downcast to primitive array float32")?
                        .values_slice_mut().context("One channel array function could not get mutable values from primitive array float32")?;
                    if cn.endian {
            ...
            DataType::Int64 => {
                if n_bytes == 8 {
                    ...
                    let data = cn.data.as_any_mut().downcast_mut::<PrimitiveBuilder<i64>>()
                        ...
                        .values_slice_mut().context("One channel array function could not get mutable values from primitive array i64")?;
                    if cn.endian {
                        for (i, mut value) in data_bytes.chunks(n_bytes).enumerate() {
                            data[i] = value
                                .read_i48::<BigEndian>()
            ...
            DataType::UInt64 => {
                if n_bytes == 8 {
                    ...
                    let data = cn.data.as_any_mut().downcast_mut::<PrimitiveBuilder<u64>>()
                        ...
                        .values_slice_mut().context("One channel array function could not get mutable values from primitive array u64")?;
                    if cn.endian {
            ...
            DataType::LargeUtf8 => {
                let mut data = LargeStringBuilder::with_capacities(cycle_count, n_bytes);
            ...
            DataType::LargeBinary => {
                let mut data = LargeBinaryBuilder::with_capacities(cycle_count, n_bytes);
                for value in data_bytes.chunks(n_bytes) {
                    data.push(value);
                }
                cn.data = data.as_box();
            ...
            DataType::FixedSizeList(_field, _size) => match cn.data.values().data_type() {
                DataType::Int8 => {
                    let mut buf = vec![0; cycle_count];
                    Cursor::new(data_bytes)
                        .read_i8_into(&mut buf)
                ...
                DataType::Float32 => {
                    ...
                    let data = array.as_any_mut().downcast_mut::<PrimitiveBuilder<f32>>()
                    ...
                    .values_slice_mut().with_context(|| format!("Read channels from bytes function could not get mutable values from primitive array complex f32, channel {}", cn.unique_name))?;
                    if n_bytes <= 2 {
                        // f16
                        if cn.endian {
                            for (i, value) in
                ...
                DataType::Float64 => {
                    ...
                    let array = cn.data.as_any_mut().downcast_mut::<FixedSizeListArray>()
                    .with_context(|| format!("Read channels from bytes function could not downcast to FixedSizeListArray, channel {}", cn.unique_name))?.mut_values();
                    let data = array.as_any_mut().downcast_mut::<PrimitiveBuilder<f64>>()
                    ...
                    .values_slice_mut().with_context(|| format!("Read channels from bytes function could not get mutable values from primitive array complex f64, channel {}", cn.unique_name))?;
                    if cn.endian {
                ...
                _ => bail!(
                    "channel {} type is not of valid array type to be read for a mdf",
                    cn.unique_name
                ),
            },
            _ => bail!(
            ...
            match cn.data.data_type() {
                DataType::Int8 => {
                    let data = cn.data.as_any_mut().downcast_mut::<PrimitiveBuilder<i8>>()
                        ...
                        .values_slice_mut().with_context(|| format!("Read channels from bytes function could not get mutable values from primitive array i8, channel {}", cn.unique_name))?;
                ...
                DataType::UInt8 => {
                    let data = cn.data.as_any_mut().downcast_mut::<PrimitiveBuilder<u8>>()
                        ...
                        .values_slice_mut().context("Read channels from bytes function could not get mutable values from primitive array u8")?;
                    for (i, record) in data_chunk.chunks(record_length).enumerate() {
                ...
                DataType::Int16 => {
                    let data = cn.data.as_any_mut().downcast_mut::<PrimitiveBuilder<i16>>()
                        ...
                        .values_slice_mut().context("Read channels from bytes function could not get mutable values from primitive array i16")?;
                    if cn.endian {
                        for (i, record) in data_chunk.chunks(record_length).enumerate() {
                            value =
                ...
                DataType::UInt16 => {
                    ...
                    .values_slice_mut().context("Read channels from bytes function could not get mutable values from primitive array u16")?;
                    if cn.endian {
                        for (i, record) in data_chunk.chunks(record_length).enumerate() {
                            value =
                ...
                DataType::Int32 => {
                    let data = cn.data.as_any_mut().downcast_mut::<PrimitiveBuilder<i32>>()
                    .with_context(|| format!("Read channels from bytes function could not downcast to primitive array i32, channel {}", cn.unique_name))?.values_slice_mut().with_context(|| format!("Read channels from bytes function could not get mutable values from primitive array i32, channel {}", cn.unique_name))?;
                    if  n_bytes == 3 {
                        if cn.endian {
                            for (i, record) in data_chunk.chunks(record_length).enumerate() {
                ...
                DataType::UInt32 => {
                    let data = cn.data.as_any_mut().downcast_mut::<PrimitiveBuilder<u32>>().with_context(|| format!("Read channels from bytes function could not downcast to primitive array u32, channel {}", cn.unique_name))?.values_slice_mut().with_context(|| format!("Read channels from bytes function could not get mutable values from primitive array u32, channel {}", cn.unique_name))?;
                    if n_bytes == 3 {
                        if cn.endian {
                            for (i, record) in data_chunk.chunks(record_length).enumerate() {
                                value = &record[pos_byte_beg..pos_byte_beg + n_bytes];
                ...
                DataType::Float32 => {
                    ...
                    .values_slice_mut().with_context(|| format!("Read channels from bytes function could not get mutable values from primitive array f32, channel {}", cn.unique_name))?;
                    if cn.endian {
                        if n_bytes == 2 {
                            for (i, record) in data_chunk.chunks(record_length).enumerate() {
                                value =
                ...
                DataType::Int64 => {
                    ...
                    .values_slice_mut().with_context(|| format!("Read channels from bytes function could not get mutable values from primitive array i64, channel {}", cn.unique_name))?;
                    if cn.endian {
                        if n_bytes == 8 {
                            for (i, record) in data_chunk.chunks(record_length).enumerate() {
                                value = &record[pos_byte_beg..pos_byte_beg + n_bytes];
                ...
                DataType::UInt64 => {
                    ...
                    .values_slice_mut().with_context(|| format!("Read channels from bytes function could not get mutable values from primitive array u64, channel {}", cn.unique_name))?;
                    if cn.endian {
                        if n_bytes == 8 {
                            for (i, record) in data_chunk.chunks(record_length).enumerate() {
                                value = &record[pos_byte_beg..pos_byte_beg + n_bytes];
                ...
                DataType::Float64 => {
                    ...
                    .values_slice_mut().with_context(|| format!("Read channels from bytes function could not get mutable values from primitive array f64, channel {}", cn.unique_name))?;
                    if cn.endian {
                        for (i, record) in data_chunk.chunks(record_length).enumerate() {
                            value =
                ...
                DataType::LargeUtf8 => {
                    ...
                    let mut values = array.values_slice_mut().context("could not get mutable reference of UTF8 array values")?.to_vec();
                ...
                DataType::LargeBinary => {
                    ...
                    let mut values = data.values_slice_mut().context("could not get mutable reference of Binary Array array values")?.to_vec();
                ...
                DataType::FixedSizeBinary(_size) => {
                    ...
                    let data = d.values_slice_mut().with_context(|| format!("failed creating MutableFixedSizeBinaryArray, channel {}", cn.unique_name))?;
                    for (i, record) in data_chunk.chunks(record_length).enumerate() {
                        data[i*n_bytes+previous_index..(i+1)*n_bytes+previous_index].copy_from_slice(&record[pos_byte_beg..pos_byte_beg + n_bytes]);
                    }
                }
                DataType::FixedSizeList(_field, size) => match cn.data.values().data_type() {
                    DataType::Float32 => {
                        let data = cn.data.values().as_any_mut().downcast_mut::<PrimitiveBuilder<f32>>()
                        .with_context(|| format!("Read channels from bytes function could not downcast to primitive array complex f32, channel {}", cn.unique_name))?
                        .values_slice_mut().with_context(|| format!("Read channels from bytes function could not get mutable values from primitive array complex f32, channel {}", cn.unique_name))?;
                        if n_bytes <= 2 {  // f16
                            if cn.endian {
                    ...
                    DataType::Float64 => {
                        let data = cn.data.values().as_any_mut().downcast_mut::<PrimitiveBuilder<f64>>()
                        .with_context(|| format!("Read channels from bytes function could not downcast to primitive array complex f64, channel {}", cn.unique_name))?
                        .values_slice_mut().with_context(|| format!("Read channels from bytes function could not get mutable values from primitive array complex f64, channel {}", cn.unique_name))?;
                        if cn.endian {
                            for (i, record) in data_chunk.chunks(record_length).enumerate() {
                                for (j, value) in record[pos_byte_beg..pos_byte_beg+size*n_bytes].chunks(n_bytes) {
                                    data[i*n_bytes+previous_index..(i+1)*n_bytes+previous_index]  = f64::from_be_bytes(
                                        value
                                            .try_into()
