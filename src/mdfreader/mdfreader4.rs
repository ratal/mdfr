            ...
            for (_dg_position, dg) in info.dg.iter_mut() {
                ...
                if dg.block.dg_data != 0 && !channel_names_to_read_in_dg.is_empty() {
                    ...
                    convert_all_channels(dg, &info.sharable)
                        .context("failed converting channels")?;
                }
            }
        }
        MdfInfo::V3(_) => {}
...
fn read_data(
    ...
    match id {
        ...
        [35, 35, 68, 90] => {
            // ##DZ
            let (mut data, block_header) = parse_dz(rdr).context("failed reading DZ block")?;
            // compressed data
            if sorted {
                // sorted data group
                for channel_group in dg.cg.values_mut() {
        ...
        for rec_pos in vlsd_channels {
            if let Some(cn) = channel_group.cn.get_mut(rec_pos) {
                ...
                if "##SD".as_bytes() == id {
                    ...
                    position = pos;
                    let (dl_blocks, pos) =
                        parser_dl4(rdr, position).context("failed parsing DL4 block")?;
...
fn read_vlsd_from_bytes(
    ...
    match cn.data.data_type() {
        ...
        DataType::LargeBinary => {
            ...
            let mut values = array
                .get_mut_values()
                .context("could not get mutable reference of Binary array values")?
                .to_vec();
            let mut offsets = array
                .get_mut_offsets()
                .context("could not get mutable reference of Binary array offsets")?
                .to_vec();
            while remaining > 0 {
                let len = &data[position..position + std::mem::size_of::<u32>()];
                let length: usize =
...
fn parser_ld4(
    ...
    if ld_blocks.len() == 1 && ld_blocks[0].ld_data().len() == 1 && channel_group.cn.len() == 1 {
        ...
        if id == "##DZ".as_bytes() {
            let (dt, block_header) = parse_dz(rdr).context("failed parsing DZ4 block")?;
        ...
        if channel_group.block.cg_inval_bytes > 0 {
            ...
            rdr.read_exact(&mut id)
                .context("could not read data block id from ld4 invalid")?;
            if id == "##DZ".as_bytes() {
                let (dt, block_header) = parse_dz(rdr).context("failed parsing DZ block")?;
        ...
        .context("failed reading DV and DI blocks")?;
    }
    Ok(position)
}
...
fn read_dv_di(
    ...
    for ld in ld_blocks {
        ...
        for data_pointer in ld.ld_invalid_data() {
            ...
            let block_length: usize;
            if id == "##DZ".as_bytes() {
                let (dt, block_header) = parse_dz(rdr).context("could not read DZ Block")?;
...
fn parser_dl4(rdr: &mut BufReader<&File>, mut position: i64) -> Result<(Vec<Dl4Block>, i64)> {
    ...
    let (block, pos) =
        parser_dl4_block(rdr, position, position).context("could not read DL4 Block")?;
    position = pos;
            ...
            if id == "##SD".as_bytes() {
                if let Some(cn) = channel_group.cn.get_mut(rec_pos) {
                    previous_index = read_vlsd_from_bytes(&mut data, cn, previous_index, decoder)
                        .context("could read VLSD from bytes")?;
                }
            } else {
                let n_record_chunk = block_length / record_length;
                if previous_index + n_record_chunk < cg_cycle_count {
...
fn parser_dl4_unsorted(
    ...
    for dl in dl_blocks {
        for data_pointer in dl.dl_data {
            ...
            .context("could not read all channels unsorted from bytes")?;
...
fn read_all_channels_unsorted(
    ...
    while position < data_block_length {
        ...
        .context("failed reading all channels unsorted from bytes")?;
    }
    Ok(())
}
                                    ...
                                    if let Some((nrecord, _)) = record_counter.get_mut(&rec_id) {
                                        match target_cn.data.data_type() {
                                            ...
                                            DataType::LargeBinary => {
                                                ...
                                                let mut values = array.get_mut_values().context("could not get mutable reference of binary array values")?.to_vec();
                                                let mut offsets = array.get_mut_offsets().context("could not get mutable reference of binary array offsets")?.to_vec();
                                                values.extend_from_slice(record);
                                                let last_offset =
                                                    offsets.last_mut().copied().unwrap_or(0)
                                                        + record.len() as i64;
                                                ...
                                                target_cn.data = BinaryArray::<i64>::try_new(
                                                    ...
                                                    values.into(),
                                                    None,
                                                ).context("failed creating BinaryArray from muted offsets and values with VLSD record")?.boxed();
                                            }
                                            _ => {
